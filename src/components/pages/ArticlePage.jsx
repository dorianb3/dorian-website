...
const { filename } = useParams();
  ...
  useEffect(() => {
    const fetchArticleContent = async () => {
      try {
        // Make a request to your API to fetch the article content based on the articleId
        const response = await fetch(`https://api.dorianbonnet.com/articles/${encodeURIComponent(filename)}`);
        if (response.ok) {
          const data = await response.json();
          // Assuming your API returns the article content in the 'content' field
          const { content, metadata } = parseArticle(data);
          setArticleContent(content);
          setMetadata(metadata);
        } else {
          // Handle the case when the article content couldn't be fetched
  ...
  return (
    <main>
        <div className="article">
            ...
            <div className='readingTime'>{readingTime} Mins <img src={schedule_black_24dp} className="logo" alt="reading logo"/></div>
